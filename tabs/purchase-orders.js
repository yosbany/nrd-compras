...
function loadPurchaseOrders() {
  ...
  purchaseOrdersListener = window.nrd.purchaseOrders.onValue((orders) => {
    ...
    if (purchaseOrdersSearchTerm.trim()) {
      ...
      const normalizedTerm = normalizeSearchText(purchaseOrdersSearchTerm.trim());
      ordersToShow = ordersToShow.filter(([id, order]) => {
        const supplierName = normalizeSearchText(order.supplierName || '');
        const status = normalizeSearchText(order.status || '');
        const orderNumber = normalizeSearchText(order.orderNumber || '');
        
        return supplierName.includes(normalizedTerm) || 
    ...
    productSearchTimeout = setTimeout(async () => {
      try {
        const [withVariants, withoutVariants] = await Promise.all([
          window.nrd.products.getAll({ withVariants: true }),
          window.nrd.products.getAll({ withVariants: false })
        ]);
        const productsArray = [...(withVariants || []), ...(withoutVariants || [])].map(p => ({
          ...p,
          id: p.variantId ? `${p.productId}_${p.variantId}` : (p.productId || p.id)
        }));
        
        const activeProducts = productsArray.filter(p => p.active !== false);
        
        const displayName = (p) => p.productName && p.variantId ? `${p.productName} - ${p.name}` : (p.name || 'Producto');
        const normalizeSearchText = window.normalizeSearchText || window.NRDCommon?.normalizeSearchText || ((t) => t.toLowerCase());
        const normalizedTerm = normalizeSearchText(searchTerm);
        const matches = activeProducts
          .filter(product => {
            const name = normalizeSearchText(displayName(product));
        ...
        searchResults.innerHTML = matches.map(product => `
          <div class="p-4 hover:bg-gray-50 active:bg-gray-100 cursor-pointer border-b border-gray-200 product-result touch-target" 
            ...
            data-product-name="${escapeHtml(displayName(product))}"
            data-product-price="${product.price || 0}">
            <div class="font-medium text-base mb-1">${escapeHtml(displayName(product))}</div>
  ...
  if (loadAllProductsBtn) {
    loadAllProductsBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const activeProducts = [...(withVariants || []), ...(withoutVariants || [])]
          .filter(p => p.active !== false)
          .map(p => ({ ...p, id: p.variantId ? `${p.productId}_${p.variantId}` : (p.productId || p.id) }));
        
        const getProductDisplayName = (p) => p.productName && p.variantId ? `${p.productName} - ${p.name}` : (p.name || 'Producto');
